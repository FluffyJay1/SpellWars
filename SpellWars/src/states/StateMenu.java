...
public class StateMenu extends BasicGameState implements ComponentListener{
	...
	Text waitingForMultiplayerText;
	...
	public void init(GameContainer container, StateBasedGame arg1){
		...
		waitingForMultiplayerText = new Text(ui, new Point(0, Game.WINDOW_HEIGHT/2 - 10), Game.WINDOW_WIDTH, 18, 20, 24, 30, new Color(255, 255, 255), "", TextFormat.CENTER_JUSTIFIED);
		waitingForMultiplayerText.setUseOutline(true);
		ui.addUIElement(waitingForMultiplayerText);
	}
	...
	public void update(GameContainer arg0, StateBasedGame arg1, int arg2)
		...
		if(multiplayerButton.hostServerButton.isPressed && !multiplayerButton.getRemove()) {
			button.setRemove(true);
			multiplayerButton.setRemove(true);
			leftSelector.setRemove(true);
			rightSelector.setRemove(true);
			try {
				Game.serverSocket = new ServerSocket(Game.SERVER_PORT);
				waitingForMultiplayerText.setText("YOUR IP ADDRESS IS: " + Inet4Address.getLocalHost().getHostAddress() + "|waiting for someone to connect...");
				Game.serverListenerThread = new ServerListenerThread(Game.serverSocket);
				Game.serverListenerThread.start();
		...
		if(multiplayerButton.connectToServerButton.isPressed && !multiplayerButton.getRemove()) {
			button.setRemove(true);
			multiplayerButton.setRemove(true);
			leftSelector.setRemove(true);
			rightSelector.setRemove(true);
			String serverAddress = JOptionPane.showInputDialog(
		            "Enter IP Address of a machine running Spell Wars that is\n" +
		            "on port " + Game.SERVER_PORT + ":");
			System.out.println("SUCC");
	        try {
				Game.socket = new Socket(serverAddress, Game.SERVER_PORT);
				...
				// TODO Auto-generated catch block
				e.printStackTrace();
				waitingForMultiplayerText.setText("error: unknownhost");
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} finally {
