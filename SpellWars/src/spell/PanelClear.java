...
public class PanelClear extends Spell {
	public static final float CLEAR_DURATION = 1.2f;
	public static final float FINAL_DURATION = 0.4f;
	float timer;
	ArrayList<Panel> panels;
	int panelsSet;
	public PanelClear(Unit owner){
		super(owner, 0, 0, "Panel Clear", "Reset all the panels on your side", "res/spell/panelclear.png", true);
	...
	public void onSpellUpdate() {
		...
		while(this.timer/CLEAR_DURATION > (float)panelsSet/(float)panels.size() && this.panelsSet < this.panels.size()) {
			this.panels.get(panelsSet).setPanelState(PanelState.NORMAL);
